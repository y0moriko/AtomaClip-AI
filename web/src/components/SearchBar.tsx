...
export default function SearchBar({ onSearch }: { onSearch: (query: string) => void }) {
  return (
    <div className="relative w-full max-w-sm ml-auto group">
      ...
      <div className="absolute -bottom-6 right-0 opacity-0 group-focus-within:opacity-100 transition-opacity flex items-center gap-1.5 pointer-events-none">
         <Sparkles className="w-3 h-3 text-indigo-500" />
         <span className="text-[10px] font-semibold text-muted-foreground">Deep Semantic Search <span className="text-indigo-500">Coming Soon</span></span>
      </div>
    </div>
  )
}
