  ...
  return (
    <>
      <motion.header 
        ...
        className="flex h-16 shrink-0 items-center gap-2 transition-[width,height] ease-linear group-has-[[data-collapsible=icon]]/sidebar-wrapper:h-12 border-b px-4 sticky top-0 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 z-50"
      >
      ...
      <div className="flex flex-1 flex-col gap-4 p-4 lg:p-8 overflow-y-auto custom-scrollbar bg-muted/10">
        {!searchQuery && (
          <motion.div 
            ...
            className="grid auto-rows-min gap-4 md:grid-cols-3 mb-4"
          >
            {[
              { view: 'all', icon: Library, label: 'Total Atoms', value: insights.length, display: view === 'all' ? insights.length : '--', sub: 'Full library', ringColor: 'ring-indigo-500', iconColor: 'text-indigo-500' },
              { view: 'recent', icon: Sparkles, label: 'Recent Atoms', value: insights.length, display: view === 'recent' ? insights.length : 'New', sub: 'Last 24 hours', ringColor: 'ring-indigo-500', iconColor: 'text-indigo-500' },
              { view: 'starred', icon: Star, label: 'Starred', value: insights.length, display: view === 'starred' ? insights.length : 'Saved', sub: 'Your favorites', ringColor: 'ring-yellow-500', iconColor: 'text-yellow-500', fill: view === 'starred' }
            ].map((card, i) => (
              <motion.div
                key={card.view}
              ...
              >
                <Card 
                  className={`bg-background border-none shadow-none ring-1 cursor-pointer transition-all ${view === card.view ? `${card.ringColor} shadow-sm` : 'ring-border/50 hover:ring-border'}`}
                ...
                >
                  <CardHeader className="p-4 flex flex-row items-center justify-between space-y-0">
                    ...
                    <card.icon className={`h-4 w-4 ${view === card.view ? card.iconColor : 'text-muted-foreground'} ${card.fill && view === card.view ? 'fill-yellow-500' : ''}`} />
                  </CardHeader>
            ...
            >
              <div className="flex flex-col items-center justify-center py-8 text-center">
                <motion.div 
                  ...
                  className="w-16 h-16 bg-gradient-to-br from-indigo-500 to-purple-600 rounded-2xl flex items-center justify-center mb-4 shadow-lg shadow-indigo-500/20 cursor-pointer"
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
                ...
                >
                  <div className="flex items-center gap-3 mb-2">
                    <motion.div 
                      className="w-8 h-8 rounded-full bg-indigo-500/10 text-indigo-600 flex items-center justify-center font-bold text-sm"
                  ...
                  <motion.div 
                    className="relative rounded-xl overflow-hidden border border-border aspect-video bg-muted/30"
              ...
              <motion.div 
                className="bg-indigo-600 rounded-3xl p-10 text-center text-white shadow-2xl shadow-indigo-500/20 overflow-hidden relative"
              ...
              >
                <motion.div 
                  className="absolute top-0 left-0 w-full h-full bg-[radial-gradient(circle_at_top_right,rgba(255,255,255,0.1),transparent)] pointer-events-none"
                  animate={{ 
                    background: [
                ...
                />
                <h4 className="text-2xl font-bold mb-3 relative z-10">Don't have the extension yet?</h4>
                <p className="text-indigo-100 mb-8 max-w-sm mx-auto relative z-10">
                ...
                <Link href="/download">
                  ...
                  >
                    <Button className="bg-white text-indigo-600 hover:bg-indigo-50 font-bold px-8 h-12 text-base rounded-full shadow-lg relative z-10">
                      <motion.div
